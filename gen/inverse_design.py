...
# ==========================================
# 1. 配置与特征工程库 (源自 v3_XGB_save.py)
# ==========================================
# 元素属性字典: [AtomicNumber, Mass, Radius, Electronegativity, IonizationEnergy, Valence]
element_data = {
...
# ==========================================
# 2. 逆向目标提取逻辑 (Target Extraction)
# ==========================================
MAX_ELEMENTS = 4  # 假设我们只关心最多4元化合物

def extract_target_vector(item):
    """提取预测目标 Y: [a, b, c, alpha, beta, gamma, Z1, frac1, Z2, frac2...]"""
    if 'structure' not in item: return None
    struct = item['structure']
    lattice = struct.get('lattice', {})
    ...
    # 1. 晶格参数 (6维)
    lattice_params = [
        lattice.get('a', 0), lattice.get('b', 0), lattice.get('c', 0),
        lattice.get('alpha', 90), lattice.get('beta', 90), lattice.get('gamma', 90)
    ]
    
    # 2. 成分参数 (转化为原子序数列表，按大小排序)
    comp = {}
    total_atoms = 0
    sites = struct.get('sites', [])
    for site in sites:
    ...
    for el, count in comp.items():
        if el in element_data:
            z = element_data[el][0] # 获取原子序数
            frac = count / total_atoms
            el_list.append((z, frac))
            
    # 按原子序数排序，保证唯一性 (例如总是 O(8)在前, Ba(56)在后)
    el_list.sort(key=lambda x: x[0])
    
    comp_vector = []
    # 填充到固定长度 (MAX_ELEMENTS)
    for i in range(MAX_ELEMENTS):
        if i < len(el_list):
            comp_vector.extend(el_list[i])
        else:
            comp_vector.extend([0, 0]) # 填充 0
            
    return lattice_params + comp_vector
...
# 4. 训练逆向回归模型 (MLP)
# ==========================================
print("\nTraining Inverse Model (MLPRegressor)...")
mlp = MLPRegressor(
...
# 2. 逐项指标分析
target_names = ['Lattice_a', 'Lattice_b', 'Lattice_c', 'Alpha', 'Beta', 'Gamma']
for i in range(MAX_ELEMENTS):
    target_names.extend([f'Element_{i+1}_Z', f'Element_{i+1}_Frac'])

metrics = []
for i, name in enumerate(target_names):
    mae = mean_absolute_error(y_test[:, i], y_pred[:, i])
    r2 = r2_score(y_test[:, i], y_pred[:, i])
    metrics.append([name, mae, r2])

metrics_df = pd.DataFrame(metrics, columns=['Target', 'MAE', 'R2_Score'])
...
# 图1: 晶格常数预测效果 (Parity Plot)
plt.figure(figsize=(15, 5))
titles = ['Lattice a', 'Lattice b', 'Lattice c']
for i in range(3):
    plt.subplot(1, 3, i+1)
    # 绘制真实值 vs 预测值
    plt.scatter(y_test[:, i], y_pred[:, i], alpha=0.6, s=15, color='blue')
    # 绘制 y=x 参考线
    min_val = min(y_test[:, i].min(), y_pred[:, i].min())
    max_val = max(y_test[:, i].max(), y_pred[:, i].max())
    plt.plot([min_val, max_val], [min_val, max_val], 'r--')
    
    plt.xlabel('Actual Value (Å)')
    plt.ylabel('Predicted Value (Å)')
    plt.title(f'{titles[i]}\nMAE={metrics_df.iloc[i]["MAE"]:.2f}, R2={metrics_df.iloc[i]["R2_Score"]:.2f}')
    plt.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig('inverse_lattice_parity.png')
print("\nPlot saved: inverse_lattice_parity.png")

# 图2: 元素种类预测准确度 (Atomic Number Z)
# 我们关注 Z1 (第一主元素) 和 Z4 (通常是 Ba, Pb 等重元素)
plt.figure(figsize=(10, 5))

# Z1 Parity
plt.subplot(1, 2, 1)
plt.scatter(y_test[:, 6], y_pred[:, 6], alpha=0.6, s=15, color='green') # Index 6 is Z1
plt.plot([0, 100], [0, 100], 'r--')
plt.xlabel('Actual Atomic Number (Z)')
plt.ylabel('Predicted Atomic Number (Z)')
plt.title(f'Element 1 Prediction\nMAE={metrics_df.iloc[6]["MAE"]:.2f}')
plt.grid(True, alpha=0.3)

# Z4 Parity
plt.subplot(1, 2, 2)
# Z4 index = 6 + 2*3 = 12
plt.scatter(y_test[:, 12], y_pred[:, 12], alpha=0.6, s=15, color='purple')
plt.plot([0, 100], [0, 100], 'r--')
plt.xlabel('Actual Atomic Number (Z)')
plt.ylabel('Predicted Atomic Number (Z)')
plt.title(f'Element 4 Prediction\nMAE={metrics_df.iloc[12]["MAE"]:.2f}')
plt.grid(True, alpha=0.3)

plt.tight_layout()
plt.savefig('invs_dgn_model/inverse_element_parity.png')
print("Plot saved: invs_dgn_model/inverse_element_parity.png")

# ==========================================
# 7. 保存模型
# ==========================================
joblib.dump(mlp, 'invs_dgn_model/inverse_design_mlp.pkl')
joblib.dump(scaler_X, 'invs_dgn_model/inverse_scaler_X.pkl')
joblib.dump(scaler_Y, 'invs_dgn_model/inverse_scaler_Y.pkl')
print("\nModel saved to 'invs_dgn_model/inverse_design_mlp.pkl'. Ready for deployment.")