...
# ==========================================
# 2. 自定义 SMOTE 函数 (无需 imblearn)
# ==========================================
def custom_smote_impl(X, y, k_neighbors=5, random_state=42):
    """
    一个无需依赖 imblearn 的原生 SMOTE 实现。
    使用 sklearn.neighbors.NearestNeighbors 进行插值。
    """
    ...
    n_class_0 = counts[classes == 0][0]
    n_class_1 = counts[classes == 1][0]
    ...
    X_min = X[y == min_class]
    n_syn = n_maj - n_min # 需要合成的数量
    
    if n_syn <= 0:
        return X, y

    print(f"Executing Manual SMOTE: Generating {n_syn} samples for class {min_class}...")
    
    # 如果样本极少 (少于 k_neighbors)，直接随机复制
    if n_min <= k_neighbors:
        indices = np.random.choice(n_min, n_syn, replace=True)
        X_syn = X_min[indices]
    else:
        # 使用 KNN 寻找邻居
        ...
        # indices 的第一列是样本自己，后面是邻居
        X_syn = []
        for _ in range(n_syn):
            # 随机选一个种子样本
            idx_seed = np.random.randint(0, n_min)
            
            # 随机选一个邻居 (排除自己)
            idx_neighbor = np.random.choice(indices[idx_seed, 1:])
            
            # 线性插值
            diff = X_min[idx_neighbor] - X_min[idx_seed]
            gap = np.random.random()
            new_sample = X_min[idx_seed] + gap * diff
            X_syn.append(new_sample)
...
# 3. Apply Manual SMOTE
X_train_bal, y_train_bal = custom_smote_impl(X_train_scaled, y_train)

# 4. Train MLP
print("Training MLP on balanced data...")
mlp = MLPClassifier(hidden_layer_sizes=(100, 50), activation='relu', solver='adam', alpha=0.01, max_iter=500, early_stopping=True, random_state=42)
...
# ==========================================
def plot_threshold_analysis(y_true, y_prob_avg, save_path='threshold_analysis.png'):
    thresholds = np.linspace(0.01, 0.99, 100)
    precisions, recalls, f1s = [], [], []
    for t in thresholds:
        y_pred_t = (y_prob_avg >= t).astype(int)
        p_denom = np.sum(y_pred_t == 1)
        p = np.sum((y_pred_t == 1) & (y_true == 1)) / p_denom if p_denom > 0 else 1.0
        r_denom = np.sum(y_true == 1)
        r = np.sum((y_pred_t == 1) & (y_true == 1)) / r_denom if r_denom > 0 else 0.0
    ...
    plt.plot(thresholds, f1s, label='F1 Score', color='red', linestyle='--')
    best_idx = np.argmax(f1s)
    plt.axvline(x=thresholds[best_idx], color='red', linestyle=':', alpha=0.5)
    plt.text(thresholds[best_idx], f1s[best_idx]+0.02, f"Max F1\nT={thresholds[best_idx]:.2f}", color='red', ha='center')
    plt.title('Threshold Analysis (Manual SMOTE)')
