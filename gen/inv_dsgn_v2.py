...
# ==========================================
# 4. 改进的模型管道 (Feature Engineering + XGBoost)
# ==========================================

# 4.1 识别需要特殊处理的特征列
# FEATURE_NAMES 索引：SpaceGroup_Number 是第 14 列 (index 14)
categorical_features = [14] 
numerical_features = [i for i in range(len(FEATURE_NAMES)) if i not in categorical_features]

# 4.2 构建预处理 Pipeline
# 数值特征：保持原样 (XGBoost 不需要 Scaler，但为了稳健可以保留)
# 类别特征 (SpaceGroup)：进行 One-Hot Encoding
...
# 4.3 定义核心模型：MultiOutputRegressor + XGBoost
# MultiOutputRegressor 会为 y 的每一列（共14列）单独训练一个 XGBRegressor
...
print("\nTraining Enhanced Multi-Output XGBoost Model...")
...
# 保存新模型
joblib.dump(model, 'invs_dgn_model/inverse_design_xgb_v4.pkl')
print("\nModel saved to 'invs_dgn_model/inverse_design_xgb_v4.pkl'")