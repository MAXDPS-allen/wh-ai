#!/usr/bin/env python3
"""
NequIP增强型铁电分类器 v6 - 高召回率版本
目标: Recall >= 99% (即使牺牲一些精确率)

策略:
1. One-Class SVM / Isolation Forest - 将FE作为正常类
2. 极低分类阈值
3. 多阶段级联分类器
4. 负样本欠采样
"""
    ...
    def _extract_features_batch(self, data: List[Dict], filename: str) -> List[np.ndarray]:
        ...
        for item in tqdm(data, desc=f"特征提取"):
            try:
                ...
                feat = self.feature_extractor.extract_advanced_features(struct, sg_num)
...
class HighRecallEvaluator:
    ...
    def train_fold(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        X_train_neg = X_train_scaled[y_train == 0]
        
        # =====================================================
        # 策略1: SMOTE + 高权重Random Forest
        # =====================================================
        print("策略1: SMOTE + Random Forest...")
        X_smote, y_smote = self._apply_smote(X_train_scaled, y_train, ratio=1.0)
        ...
        # =====================================================
        # 策略2: One-Class SVM (只用正样本训练)
        # =====================================================
        print("策略2: One-Class SVM...")
        ocsvm = OneClassSVM(kernel='rbf', nu=0.1, gamma='scale')
        ocsvm.fit(X_train_pos)
        ocsvm_scores = ocsvm.decision_function(X_test_scaled)
        ocsvm_proba = 1 / (1 + np.exp(-ocsvm_scores))  # Sigmoid转换
        
        # =====================================================
        # 策略3: Isolation Forest (将FE视为"正常")
        # =====================================================
        print("策略3: Isolation Forest...")
        iso = IsolationForest(
            n_estimators=200,
            contamination=0.1,  # 10%异常（训练集中只有正样本）
            random_state=self.config.random_state,
            n_jobs=-1
        )
        iso.fit(X_train_pos)
        iso_scores = iso.decision_function(X_test_scaled)
        iso_proba = 1 / (1 + np.exp(-iso_scores * 5))  # Sigmoid转换，放大差异
        
        # =====================================================
        # 策略4: 欠采样 + Extra Trees
        # =====================================================
        print("策略4: 欠采样 + Extra Trees...")
        # 欠采样负样本
        undersample_ratio = min(5, ratio)  # 最多5:1
        n_neg_sample = int(n_pos * undersample_ratio)
        np.random.seed(self.config.random_state)
        neg_indices = np.random.choice(len(X_train_neg), n_neg_sample, replace=False)
        X_under = np.vstack([X_train_pos, X_train_neg[neg_indices]])
        y_under = np.concatenate([np.ones(n_pos), np.zeros(n_neg_sample)])
        
        et = ExtraTreesClassifier(
            n_estimators=500,
            class_weight={0: 1, 1: undersample_ratio * 2},
            random_state=self.config.random_state,
            n_jobs=-1
        )
        et.fit(X_under, y_under)
        ...
        dist_proba = 1 / (1 + distances / distances.std())
    ...
    def _apply_smote(self, X: np.ndarray, y: np.ndarray, ratio: float = 1.0) -> Tuple[np.ndarray, np.ndarray]:
        """SMOTE过采样"""
        minority_mask = y == 1
...
def main():
    print("=" * 70)
    print("NequIP增强型铁电分类器 v6 - 高召回率版本")
    print("目标: Recall >= 99%")
    print("=" * 70)
    ...
    evaluator = HighRecallEvaluator(config)
    ...
    # 保存结果
    results_df.to_csv(config.report_dir / "cv_results_high_recall.csv", index=False)
    print(f"\n已保存结果到: {config.report_dir / 'cv_results_high_recall.csv'}")
