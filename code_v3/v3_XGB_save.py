...
import joblib  # <--- [新增] 导入 joblib 用于保存模型
...
print("\n====== Saving Model & Scaler ======")
# 保存模型
model_filename = 'model_v3/v_3_fe_model.pkl'
joblib.dump(clf, model_filename)
print(f"Model saved to: {model_filename}")

# 保存标准化器 (Scaler)
# 注意：这非常重要！如果你不保存scaler，将来用新数据预测时就无法进行同样的归一化，预测结果会出错。
scaler_filename = 'model_v3/v_3_fe_scaler.pkl'
joblib.dump(scaler, scaler_filename)
print(f"Scaler saved to: {scaler_filename}")

# 也可以选择保存特征名称，方便后续对应
feature_names_file = 'model_v3/v_3_fe_features.json'
