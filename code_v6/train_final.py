...
目标: 在保持高 Accuracy 的同时最大化 Recall

策略:
1. 使用64维 NequIP 特征 (验证有效)
2. Transformer 架构
3. 更激进的正样本采样 (oversample 比例更高)
4. 阈值校准
5. 模型集成
"""
...
def evaluate(model, val_loader, device):
    ...
    # 找最优阈值使得 recall >= 0.99 的情况下 accuracy 最高
    best_acc_high_recall = 0
    best_thresh_high_recall = 0.1
    
    for thresh in np.arange(0.01, 0.5, 0.01):
        preds = (all_probs >= thresh).astype(int)
        rec = recall_score(all_labels, preds, zero_division=0)
        acc = accuracy_score(all_labels, preds)
        if rec >= 0.99 and acc > best_acc_high_recall:
            best_acc_high_recall = acc
            best_thresh_high_recall = thresh
    
    results = {
        'auc': auc, 
        'best_thresh_high_recall': best_thresh_high_recall,
        'best_acc_high_recall': best_acc_high_recall
    }
...
def main():
    print("=" * 70)
    print("最终版铁电分类器 - 高 Recall 优化")
    print("目标: 在保持高 Accuracy 的同时最大化 Recall")
    print("=" * 70)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y), 1):
        ...
        # 更激进的加权采样器 - 使正样本被采样的概率更高
        class_counts = np.bincount(y_train.astype(int))
        # 使正样本权重更高
        class_weights = np.array([1.0, class_counts[0] / class_counts[1] * 2.0])  # 2倍加权
        ...
        # 损失函数 - 使用更高的正样本权重
        pos_weight = len(y_train[y_train == 0]) / len(y_train[y_train == 1]) * 1.5  # 1.5倍加权
        ...
        all_models.append((model.state_dict().copy(), scaler))
        
        # 最终评估
        final_results, probs, labels = evaluate(model, val_loader, device)
        
        print(f"\nFold {fold} 结果:")
        print(f"  ROC-AUC: {final_results['auc']:.4f}")
        print(f"  高Recall最优阈值: {final_results['best_thresh_high_recall']:.4f}")
        print(f"  高Recall时Accuracy: {final_results['best_acc_high_recall']:.4f}")
    ...
    print("\n" + "=" * 70)
    print("寻找最佳阈值 (目标: Recall >= 99% 且 Accuracy 最高)")
    print("=" * 70)
    ...
    for thresh in [0.05, 0.10, 0.15, 0.20, 0.30]:
        ...
        marker = "★" if avg_recall >= 0.99 else ""
        print(f"\n阈值 {thresh}: {marker}")
    ...
    df.to_csv(output_dir / 'cv_results_final.csv', index=False)
    print(f"\n结果已保存到: {output_dir / 'cv_results_final.csv'}")
